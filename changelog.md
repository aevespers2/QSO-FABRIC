...
- 2026-07-20 — Added a lowest-coupling format-governance candidate: a small neutral envelope/conformance layer, repository-owned payload profiles, Repository `1` authority records, and Fabric-owned experiment contracts.
...
### Architecture
- 2026-07-20 — Added `docs/FORMAT_GOVERNANCE.md` with a portfolio ownership matrix, minimal common-envelope candidate, mutation-dimension repair, serialization-status rules, registry governance, pairwise/triple-overlap fixtures, migration requirements, and disposition options for PR #17.
- 2026-07-20 — Identified that PR #17's mutation classes combine storage behavior, proposal source, authority, derivation, lifetime, and governance; proposed independent dimensions so a descriptive class cannot silently grant authority.
- 2026-07-20 — Identified a serialization-status obstruction: PR #17 names canonical CBOR normative while the verified implementation is the canonical JSON authoring profile; CBOR, package, stream, and signature profiles remain proposed pending conformance evidence.
...
### Security
- 2026-07-20 — Clarified that envelopes, hashes, signatures, Witness records, immutable routing records, successful validation, mutation labels, or successful execution do not independently grant authority.
...
- 2026-07-20 — Required unsupported canonicalization, wrong profile, wrong identity, stale, replayed, partial, corrected, revoked, and authority-denied cases to fail closed across pairwise and triple-overlap fixtures.
...
### Documentation
- 2026-07-20 — Added `docs/FORMAT_GOVERNANCE.md` and linked it from the README, task chain, punch list, and release plan.
- 2026-07-20 — Split the former P4 format/QSIO task into P4A ownership decomposition and P4B gluing validation.
...
- Format admission is now split into ownership decomposition and gluing validation; neither gate is satisfied by PR #17's local validator or mergeability.
- Format and QSIO integration remain blocked until ownership, field vocabulary, lifecycle/mutation semantics, ledger classes, capability separation, privacy, signing, correction, and recovery are approved with deterministic witnesses.
