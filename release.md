...
Status: `BLOCKED — UNVERIFIED RUNTIME, UPSTREAM CONTRACTS, AND FORMAT/AUTHORITY GLUING`
...
The repository also has concurrent candidates for a disabled QSIO adapter (PR #16) and a QSO file-format subsystem (PR #17). These introduce unresolved ownership across QSO-FABRIC, QSO-GENOMES, QuantumStateObjects, `qsio-kernel`, Repository `1`, Bridge, QSO-SEEKER, and review interfaces.

The following documents govern review:

- [`punchlist.md`](punchlist.md) — executable and evidence work;
- [`docs/OBSTRUCTION_AND_GLUING.md`](docs/OBSTRUCTION_AND_GLUING.md) — obstruction and witness ledger;
- [`docs/FORMAT_GOVERNANCE.md`](docs/FORMAT_GOVERNANCE.md) — format ownership decomposition and PR #17 disposition options.

PR #1, which proposed scheduled owner-wide repository mutation, was closed on 2026-07-16. No repository-bootstrap automation is included, approved, scheduled, or deployed.
...
- the canonical owner of all QSO envelopes, registries, genomes, runtime lifecycle, kernel semantics, or transport contracts;
...
- A neutral QSO envelope, registry, and conformance package requires its own approved owner and release identity.
- PR #17's `0.1` label is a candidate-format version, not an accepted portfolio standard.
- Canonical JSON is the only implemented authoring/validation profile observed in PR #17; canonical CBOR, package, stream, and signature profiles remain proposed until independent byte-level conformance exists.
...
- Documentation of QSO-FABRIC's role, architecture, onboarding, candidate semantics, obstruction ledger, format-governance decomposition, release punch list, release gates, and A.L.I.S.T.A.I.R.E. boundary.
...
- Treating an envelope, hash, signature, Witness record, accepted routing record, mutation label, or successful command as permission to act.

## Selected completed documentation work
...
- The format-governance guide proposes a small neutral envelope and conformance layer, repository-owned profiles, Repository `1` authority records, and explicit migration/provenance requirements.
- The release punch list decomposes P0–P5, security, privacy, governance, recovery, documentation evidence, and release stop conditions.
...
| Format ownership | BLOCKED | Neutral envelope/registry owner, repository profiles, canonicalization status, mutation decomposition, migration, privacy, and recovery are approved. |
| Format and QSIO gluing | BLOCKED | Pairwise and triple-overlap fixtures prove all selected edges commute and fail closed. |
| Documentation | PARTIAL | Core documentation exists; branch ancestry reconciliation, exact-head link validation, Pages-setting review, rendered evidence, and publication approval remain incomplete. |
...
3. assigns canonical owners for shared fields, algorithms, registries, and lifecycle transitions;
4. decomposes common envelope concerns from repository-specific profiles;
5. rebases and reconciles every shared invariant;
6. produces exact-head deterministic, security, privacy, failure, rollback, and provenance evidence;
7. updates task chain, punch list, release, changelog, migration guidance, and public documentation;
8. receives explicit acceptance.
...
Each witness must include positive, malformed, unsupported-version, stale, replay, wrong-identity, hash-mismatch, authority-denied, partial-failure, correction, revocation, and rollback behavior as applicable.
...
- Accepted common-envelope and profile contracts if a format candidate is included.
- Canonicalization algorithms and byte-level vectors for every accepted serialization profile.
...
- Migration/provenance map for any moved PR #17 content.
- Recovery reconstruction manifest pinning contracts, objects, checkpoints, capabilities, and evidence hashes.

## Rollback criteria

Withdraw or roll back the candidate if seeded runs diverge outside documented nondeterministic fields; ledger tampering is undetected; limits, consent, freeze, revocation, or authority can be bypassed; timeout/interruption corrupts evidence; upstream contracts drift; format ownership remains ambiguous; proposed encodings are represented as implemented; a local ledger is confused with canonical state; mutation terminology becomes self-authorizing; private objectives or identities are exposed; candidate assumptions conflict; recovery cannot reconstruct the accepted state; or artifact hashes differ.
...
- Canonical envelope/registry ownership, profile boundaries, lifecycle and mutation authority, authoritative ledger classes, signing, privacy, correction, retention, capability, and recovery ownership remain unresolved.
- The documentation branch must be reconciled with current trusted `main` ancestry and all checks regenerated at the resulting immutable head.
...
- 2026-07-20: Added the QSO format governance and ownership guide; split format admission into ownership decomposition and gluing-validation gates. No owner, format, encoding, mutation class, adapter, or authority was accepted.
- 2026-07-20: Added the obstruction and gluing ledger and release punch list; documented PR #16 and PR #17 as blocked candidates pending ownership and compatibility witnesses.
